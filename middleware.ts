...
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith("/dashboard")) {
    ...
    if (segments.length === 1 && segments[0] === "dashboard") {
      const url = request.nextUrl.clone();
      url.pathname = defaultDashboardPath(role);
      return redirectWithCookies(supabaseResponse, url);
    }
    ...
    if (
      ...
      segments.length === 2 &&
      isUserRole(segments[1]) &&
      segments[1] === role
    ) {
      const target = defaultDashboardPath(role);
      if (pathname !== target) {
        const url = request.nextUrl.clone();
        url.pathname = target;
        return redirectWithCookies(supabaseResponse, url);
  ...
  if (pathname === "/login" && user) {
    ...
    if (r && typeof r === "string" && isUserRole(r)) {
      const url = request.nextUrl.clone();
      url.pathname = defaultDashboardPath(r);
      return redirectWithCookies(supabaseResponse, url);
    }
  }
